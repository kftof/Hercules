...
OnGuildBreak:
	...
	sleep 7000;
	announce "Le château de [ "+getcastlename(strnpcinfo(2))+" ] a été abandonné.",0;
...
OnStartArena:
	...
	announce "La guilde [ "+getguildname(.@GID)+" ] a conquis le château de "+getcastlename(strnpcinfo(4))+" !",bc_all|bc_woe;
	mapannounce strnpcinfo(4),"L'emperium a été détruit.",bc_map,"0x00FF00",FW_NORMAL,20,0,40;
	...
	if (agitcheck2()) {
		...
		mapannounce strnpcinfo(4),"Reconstruisez vos défenses et gardien pour sécuriser votre château !",bc_map,"0x00FF00",FW_NORMAL,20,0,40;
	}
	end;
}
